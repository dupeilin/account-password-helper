...
第二个坑：**菜单父项标题过长会把二级菜单挤出屏幕**。Chrome 只在同一上下文里有多个可见顶级项时，才把扩展的菜单项折叠到以扩展全名为标题的二级菜单下；只有一个顶级项时，它直接用你注册的那个父项。而这个扩展在商店里的名字带副标题（"账号密码管理助手 - 本地加密密码管理器与自动填充及两步验证"），一旦用它当父级，标题宽到把二级菜单顶出可视区，用户右键后看到的是半截菜单。所以现在显式注册单个父项（输入框上下文里叫「填充账号密码」，页面空白处叫「账号密码管理助手」），层级深度和折叠方案完全一致，仍是二级。父项必须先于子项创建——Chrome 要求 `parentId` 指向的项已经存在；语言切换时整套菜单重建，因为菜单标题是按用户语言渲染的。
...
回头看，四条链路都遵守同一套约束：**不加开关、除 `contextMenus` 外不加权限、不改存储结构与加密格式、不动既有条目的语义**；新增的用户可见文案全部中英双语成对提交；每个功能都带回归测试，全仓库 642 项自动化测试（分布在 55 个测试文件）与 `pnpm build`（Chromium）+ `pnpm build:firefox`（Firefox）双端构建作为验收线。

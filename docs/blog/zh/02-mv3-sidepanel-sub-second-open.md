...
[Account Password Helper](https://github.com/liaolongdong/account-password-helper) 是一款本地密码管理器扩展，侧边栏是它的最高频入口（解锁、搜索、填充都在这）。我们给它定了一个硬性 SLA：**所有场景（会话有效/失效、浏览器冷启动、快速重启）侧边栏必须 1 秒内打开、无白屏**。最终结果：缓存快路径 20-50ms 拿到数据。
...
- `chrome.alarms` 的官方限制是**最快每 30 秒触发一次**（以未打包方式加载做调试时不限频率），所以 0.5 分钟已经是打包扩展能写到的最小周期，别指望秒级精度——秒级的连续性由 20 秒心跳层负责，闹钟层只兜"起死回生"；
...
**平台差异化。** Windows 的主要瓶颈是 Defender 对扩展目录的逐文件扫描，文件读得越多、扫得越久，所以 Windows 走全量预热（约 25 个文件），把扫描成本提前摊掉；Mac 等 Unix 平台没有这个问题，只预热白名单里的约 15 个核心文件。**触发时机**：扩展安装/更新、浏览器启动、窗口聚焦、Tab 激活、保活闹钟 tick、侧边栏打开 2 秒后——都是"用户可能要打开侧边栏"的前置信号。
...
2. **后台 `GET_INITIAL_DATA` 内存缓存**：后台在 SW 启动 500ms 后就把密码缓存回温，走消息通道拿。这条路径上有一个 **800ms 竞速门**：门内返回就直接用；冷 SW 超过 800ms 才回话时，本地兜底路径已经并行跑起来了，迟到的那份原始响应不会被丢弃，它继续和本地路径竞速，由一个共用的"已采纳"标记保证同一响应只被处理一次；
3. **本地 storage 直读**：与上面两条同一时刻并行发起，侧边栏自己读 `storage.local` 解密，设 3000ms 超时——这条超时是最后的安全网，宁可先给出锁屏界面，也不让用户对着空面板干等。

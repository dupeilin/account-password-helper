...
>
> 🔎 **⑥ 明确避开三个查证后判定不适用的写法**：待确认登录凭据在 `sessionStorage` 的实现是 **XOR + base64、密钥与密文同处**（`entrypoints/content/LoginAutoSave.ts:37-58`，注释自陈目的是「提高自动化 XSS 窃取成本」），因此**不写**「待确认凭据以加密形式暂存」；`utils/passwordGenerator.ts:72-76` 的 `secureRandomInt` 取模未做 rejection sampling，**不写**「无偏」；词库出处只到「内置 3,080 词英文词库」，**不写** EFF / Diceware 词表。
>
...
> ✅ **2026-09-13 重制（推荐上传这一组）**：`assets/cws-store/screen-*.png`，**中英各 9 张**，**2560×1600（即 1280×800 @2x，正好是官方规格的 2 倍）**。每张顶部为品牌渐变标题带（`#0A1A38 → #123E77`，与 Marquee / 小推广图同源；左侧一条品牌色竖条做视觉锚点），下方为界面演示，全部使用占位演示数据（`example.com` / `*.example.com`，**不含任何真实账号、真实邮箱或第三方品牌**）。
>
...
>
> ⚠️ **每个语言页的截图上限是 5 张**，上表有 9 张候选，**最终选哪 5 张由上传时定**。脚本侧建议取 `1 / 2 / 3 / 4 / 9`（前四张各占一个独立卖点，第 9 张是唯一展示**页内**体验的构图，与其余「侧边栏 / 管理页」互补）；要换就换 `7`（收藏 + 标签 + 网站图标）或 `8`（信息密度最高的详情抽屉）。**若要砍一张，先砍第 6 张**（本地加密画面最朴素，且该主张在摘要与说明里已有文字承载）。
>
> 📌 第 7、8 张依赖 `seed.mjs` 的**收藏**与**改密**步骤（详情抽屉的「密码修改历史」小节只在有条目被改过密码后才渲染），只跑导入是拍不出来的。
>
> ⚠️ **标题带文案的合规约束与商店文案完全一致**：零竞品品牌名、零绝对化表述（不写「零联网 / 100% offline / 数据不出浏览器」——扩展每 6 小时有一次不携带用户数据的匿名版本检查）。改文案后必须重新跑一遍本节「生成方式」的脚本。
>
> 🚫 **旧的 `01-*.png` ~ `12-*.png` 不要再上传商店**：那 12 张拍摄于 2026-07-29 的 **v2.12.0**，界面版本徽章过期，其中 `01-master-password.png` 还带着已删除的「严禁……后果自负」旧声明，且含真实账号邮箱。**README 与 README.en.md 已于 2026-09-11 改用新的 `assets/cws-store/screen-*.png`（中英各 9 张，英文页取 `-en` 版），不再引用这批旧图**；它们若仍出现在其他文档或页面，需另行重截（见 `docs/exposure-status.md` 残留待办）。
...
| 10  | 商店列表质量     | 标题/描述/截图完整高质量            | 🔄 文案已按驳回重写待审；截图已于 2026-09-13 用 `scripts/store-shots/` 重制（中英各 9 张候选），**徽章仍是 v3.7.0，改版本后需重跑** |
...
- [ ] 分类选择 Productivity
- [ ] 商店图标已上传（128×128）
- [ ] 截图已上传（**中文页与 English (United States) 页各 5 张**，从 `assets/cws-store/screen-*.png` 的 9 张候选里选；**不要再用旧的 `01-*.png` ~ `12-*.png`**）
- [ ] 截图版本徽章与提交包一致（当前批次徽章为 v3.7.0；`package.json` 改到目标版本后 `pnpm build` 并重跑 `scripts/store-shots/`）
